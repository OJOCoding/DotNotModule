    ...
    public class CTableFormStateLoaded: CTableFormState, IFormState
    {
        ...
        public CTableFormStateLoaded(CTableFormContext p_oContext) : base(p_oContext)
        {
            ...
            if (this.context.LoadTable != null)
                this.context.LoadTable.Enabled = true;   // The user is allowed to reload a loaded table

            if (this.context.SaveTable != null)
                this.context.SaveTable.Enabled = false;  // The use is not allowed to save a loaded table without any changes
        }
        // --------------------------------------------------------------------------------------
